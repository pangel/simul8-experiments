  ...
  class OpenSearchResponse 
    ...
    def parse_rss(xml)
      ...
      nodes.each do |item|
        ...
        if xpath_first(item, "dc:identifier", namespaces) != nil
          _isbn_13 = xpath_get_text(xpath_first(item, "dc:identifier", namespaces)).tr('urn:ISBN:', '')
        end

        @records << {:title => _title, :author => _author, :link => _link, 
            :id => _id, :citation => _citation, :summary => _summary, :xml => item.to_s, :isbn => _isbn_13}
      end
    end
